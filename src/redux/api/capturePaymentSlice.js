...
import { paymentApi } from "./pyamentApi"
...
export async function BuyCourse(
  ...
  try {
    ...
    const orderResponse = await paymentApi(
      "POST",
      COURSE_PAYMENT_API,
      {
        courseId, // sending courseId instead of courses
      },
      {
...
async function verifyPayment(bodyData, token, navigate, courseId) {
  ...
  try {
    const response = await paymentApi("POST", COURSE_VERIFY_API, bodyData, {
      Authorization: `Bearer ${token}`,
    })

...
function TopRatedCourses({ metaData }) {
  ...
  const generateStructuredData = () => {
    const structuredData = {
      ...
      "@type": "ItemList",
      "name": "Top Rated Online Courses",
      "description": "Discover our highest-rated educational content and online courses",
      ...
      "itemListElement": courses.map((course, index) => ({
        ...
        "item": {
          "@type": "Course",
          "@id": `${window.location.origin}/course/${course._id}`,
          "name": course.title || course.name,
          "description": course.description,
          "provider": {
            "@type": "Organization",
            "name": "Your Platform Name" // Replace with your actual platform name
          },
          "aggregateRating": {
  ...
  if (error) {
    return (
      <section className="mx-2 sm:mx-4 lg:mx-6 xl:mx-8" role="alert" aria-labelledby="error-heading">
        <Card className="border-red-200 dark:border-red-800">
          <CardContent className="p-4 sm:p-6 text-center space-y-3 sm:space-y-4">
            <div className="p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 rounded-full w-fit mx-auto">
              <TrendingUp className="h-6 w-6 sm:h-8 sm:w-8 text-red-600 dark:text-red-400" aria-hidden="true" />
            </div>
            <div className="space-y-1 sm:space-y-2">
              <h3 id="error-heading" className="text-lg sm:text-xl font-semibold">Unable to load courses</h3>
  ...
  return (
    <>
      ...
      <Helmet>
        <title>Top Rated Online Courses - Best Educational Content</title>
        <meta name="description" content={`Discover our highest-rated educational content with an average rating of ${averageRating}. Explore ${courses.length} top-rated courses from expert instructors.`} />
        <meta name="keywords" content="top rated courses, online education, best courses, e-learning, educational content, online learning" />
        <meta property="og:title" content="Top Rated Online Courses - Best Educational Content" />
        <meta property="og:description" content={`Discover our highest-rated educational content with an average rating of ${averageRating}. Explore premium courses from expert instructors.`} />
        <meta property="og:type" content="website" />
        <link rel="canonical" href={`${window.location.origin}${window.location.pathname}#top-rated-courses`} />
      ...
      <section
        ...
        id="top-rated-courses"
        aria-labelledby="top-courses-heading"
      >
        <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          {/* Header Section with Semantic Structure */}
          <header className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 md:mb-8">
            <div className="space-y-1 sm:space-y-2">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="p-1.5 sm:p-2 bg-gradient-to-br from-yellow-100 to-orange-100 dark:from-yellow-900/30 dark:to-orange-900/30 rounded-lg">
                  <Award className="h-5 w-5 sm:h-6 sm:w-6 md:h-7 md:w-7 text-yellow-600 dark:text-yellow-400" aria-hidden="true" />
                </div>
                <div>
                  <h2
                    id="top-courses-heading"
                    className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-bold tracking-tight"
                  >
                    Top Rated Courses
                  </h2>
            ...
            <nav className="hidden md:flex items-center gap-2" role="navigation" aria-label="Course carousel navigation">
              <Button
                variant="outline"
                size="sm"
                onClick={scrollLeft}
                ...
                aria-label="View previous courses"
              >
          ...
          {/* Stats Banner with Semantic Structure */}
          {!loading && courses.length > 0 && (
            <aside className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6 md:mb-8" aria-label="Course statistics">
              <Card className="p-2 sm:p-3 md:p-4">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                  <div className="p-1 sm:p-1.5 md:p-2 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg flex-shrink-0">
                    <Star className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-yellow-600 dark:text-yellow-400" aria-hidden="true" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm text-muted-foreground truncate">
                      Avg Rating
                    </p>
                    <p className="text-sm sm:text-lg md:text-xl font-bold" aria-label={`Average rating: ${averageRating} stars`}>
                      {averageRating}
                    </p>
                  </div>
                </div>
              ...
              <Card className="p-2 sm:p-3 md:p-4">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                  <div className="p-1 sm:p-1.5 md:p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex-shrink-0">
                    <Users className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-blue-600 dark:text-blue-400" aria-hidden="true" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm text-muted-foreground truncate">
                      Total Students
                    </p>
                    <p className="text-sm sm:text-lg md:text-xl font-bold" aria-label={`Total students: ${metaData ? metaData.totalEnrolledMaterials : 'N/A'} plus`}>
              ...
              <Card className="p-2 sm:p-3 md:p-4 col-span-1 xs:col-span-2 sm:col-span-1">
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3">
                  <div className="p-1 sm:p-1.5 md:p-2 bg-green-100 dark:bg-green-900/30 rounded-lg flex-shrink-0">
                    <TrendingUp className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 text-green-600 dark:text-green-400" aria-hidden="true" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm text-muted-foreground truncate">
                      Featured Courses
                    </p>
                    <p className="text-sm sm:text-lg md:text-xl font-bold" aria-label={`Featured courses: ${metaData ? metaData.totalCourses : 'N/A'} plus`}>
          ...
          <main className="relative">
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6" role="status" aria-label="Loading courses">
                ...
                <span className="sr-only">Loading top rated courses...</span>
              </div>
            ) : courses.length > 0 ? (
              <>
                ...
                <div className="block xs:hidden">
                  ...
                  <div className="space-y-3" role="list" aria-label="Top rated courses">
                    {courses.slice(0, 3).map((course, index) => (
                      <article key={course._id} className="w-full" role="listitem">
                ...
                <div className="hidden xs:block">
                  <div
                    ...
                    role="list"
                    aria-label="Top rated courses carousel"
                    style={{
                      scrollbarWidth: 'none',
                          ...
                          {index < 3 && (
                            <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-10">
                              <Badge
                                ...
                                aria-label={`Number ${index + 1} top rated course`}
                              >
                                <Star className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 fill-current" aria-hidden="true" />
                                #{index + 1} Rated
                              </Badge>
                            </div>
                          )}
                      ...
                      <Card className="h-full border-2 border-dashed border-primary/30 hover:border-primary/50 transition-all duration-300 cursor-pointer group">
                        <CardContent className="p-3 sm:p-4 md:p-6 lg:p-8 flex flex-col items-center justify-center text-center h-full min-h-[200px] sm:min-h-[250px] md:min-h-[300px] space-y-2 sm:space-y-3 md:space-y-4">
                          <div className="p-2 sm:p-3 md:p-4 bg-primary/10 rounded-full group-hover:bg-primary/20 transition-colors">
                            <ArrowRight className="h-5 w-5 sm:h-6 sm:w-6 md:h-8 md:w-8 text-primary group-hover:translate-x-1 transition-transform" aria-hidden="true" />
                          </div>
                          <div className="space-y-1 sm:space-y-2">
                  ...
                  <nav className="flex md:hidden justify-center mt-3 sm:mt-4 gap-2" role="navigation" aria-label="Mobile course carousel navigation">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={scrollLeft}
                  ...
                  <nav className="flex justify-center mt-3 sm:mt-4 md:mt-6 gap-1 sm:gap-2" role="navigation" aria-label="Course carousel pagination">
              ...
              /* Empty State with Semantic Structure */
              <Card role="status" aria-live="polite">
                <CardContent className="p-4 sm:p-6 md:p-8 lg:p-12 text-center space-y-3 sm:space-y-4 md:space-y-6">
                  <div className="p-2 sm:p-3 md:p-4 bg-muted rounded-full w-fit mx-auto">
                    <TrendingUp className="h-6 w-6 sm:h-8 sm:w-8 md:h-10 md:w-10 lg:h-12 lg:w-12 text-muted-foreground" aria-hidden="true" />
                  </div>
                  <div className="space-y-1 sm:space-y-2">
